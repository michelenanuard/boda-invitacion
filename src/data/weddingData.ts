import type { WeddingData } from '../types/wedding'

export const weddingData: WeddingData = {
  "brideName": "Ana Kary",
  "groomName": "Marcos",
  "coupleDisplayName": "Ana Kary & Marcos",
  "weddingDate": "2026-08-30",
  "weddingTime": "17:30",
  "displayDate": "domingo 30 de agosto de 2026",
  "hashtag": "#AnaKary&Marcos",
  "heroTitle": "Ana Kary & Marcos",
  "heroSubtitle": "Toda gran historia de amor merece un hermoso comienzo.",
  "heroImage": "/wedding-images/hero.jpg",
  "ceremony": {
    "title": "Ceremonia y recepción",
    "time": "4:30 p. m.",
    "venue": "Palm Court",
    "address": "Renaissance Santo Domingo Jaragua Hotel & Casino, avenida George Washington núm. 367, frente al emblemático Malecón de Santo Domingo.",
    "mapUrl": "https://www.google.com/maps/place/Renaissance+Santo+Domingo+Jaragua+Hotel+%26+Casino/@18.4622715,-69.9024036,1041m/data=!3m1!1e3!4m9!3m8!1s0x8eaf8829a31364e5:0xabe5b14bcd1f354b!5m2!4m1!1i2!8m2!3d18.4622715!4d-69.8998287!16s%2Fm%2F0myxpqq?entry=ttu&g_ep=EgoyMDI2MDYxMC4wIKXMDSoASAFQAw%3D%3D",
    "note": "Agradecemos su puntualidad y le invitamos a llegar 20 minutos antes."
  },
  "reception": {
    "title": "Recepción",
    "time": "",
    "venue": "Salón Anacaona III",
    "description": "Hemos preparado cada momento con amor, desde nuestro \"para siempre\" hasta el último brindis.",
    "address": "Renaissance Santo Domingo Jaragua Hotel & Casino, avenida George Washington núm. 367, frente al emblemático Malecón de Santo Domingo.",
    "mapUrl": "https://www.google.com/maps/place/Renaissance+Santo+Domingo+Jaragua+Hotel+%26+Casino/@18.4622715,-69.9024036,1041m/data=!3m1!1e3!4m9!3m8!1s0x8eaf8829a31364e5:0xabe5b14bcd1f354b!5m2!4m1!1i2!8m2!3d18.4622715!4d-69.8998287!16s%2Fm%2F0myxpqq?entry=ttu&g_ep=EgoyMDI2MDYxMC4wIKXMDSoASAFQAw%3D%3D",
    "note": ""
  },
  "bride": {
    "name": "Ana Kary Lapaix",
    "role": "bride",
    "roleLabel": "La novia",
    "image": "/wedding-images/bride.jpg",
    "description": "Con la bendición de Dios y el amor de nuestras familias, damos el paso más importante de nuestras vidas.",
    "parents": "Hija de Carlos Lapaix y María Encarnación"
  },
  "groom": {
    "name": "Marcos Michelén",
    "role": "groom",
    "roleLabel": "El novio",
    "image": "/wedding-images/groom.jpg",
    "description": "Con la bendición de Dios y el amor de nuestras familias, damos el paso más importante de nuestras vidas.",
    "parents": "Hijo de Winston Michelén y Maribel Ramírez"
  },
  "story": [
    {
      "year": "2020",
      "title": "Primer encuentro",
      "description": "Una tarde tranquila, una mesa compartida y una conversación que ninguno de los dos quiso terminar."
    },
    {
      "year": "2021",
      "title": "Primera cita",
      "description": "Cena, risas y la certeza de haber encontrado a alguien con quien todo se sentía natural."
    },
    {
      "year": "2025",
      "title": "La propuesta",
      "description": "Bajo luces cálidas y con la familia cerca, llegó la pregunta que cambió el rumbo de nuestra historia."
    },
    {
      "year": "2026",
      "title": "El gran día",
      "description": "Celebraremos el amor, la familia y el inicio de una nueva vida juntos."
    }
  ],
  "gallery": [
    {
      "id": "paseo",
      "src": "/wedding-images/gallery-1.jpg",
      "alt": "Pareja caminando en una boda",
      "caption": ""
    },
    {
      "id": "decoracion",
      "src": "/wedding-images/gallery-2.jpg",
      "alt": "",
      "caption": ""
    },
    {
      "id": "venue",
      "src": "/wedding-images/gallery-3.jpg",
      "alt": "",
      "caption": "Si hoy soy mejor hombre, es porque camino contigo - Marcos Michelén "
    },
    {
      "id": "pastel",
      "src": "/wedding-images/gallery-4.jpg",
      "alt": "",
      "caption": "Tú has sido el refugio de mis miedos y el impulso de mis sueños. — Ana Kary Lapaix"
    }
  ],
  "galleryVideo": {
    "title": "Nuestro video",
    "caption": "Un instante para revivir la emoción de este capítulo",
    "poster": "/wedding-images/video-poster.jpg",
    "src": ""
  },
  "testimonials": [
    {
      "name": "Carlos Lapaix",
      "relation": "Padre de la novia",
      "message": "El amor es uno de los regalos más hermosos que Dios nos concede. Que su presencia sea siempre el fundamento de su hogar y la guía de cada paso que den juntos."
    },
    {
      "name": "María Encarnación",
      "relation": "Madre de la novia",
      "message": "Me hace muy feliz celebrar el inicio de esta nueva etapa, fruto de un amor genuino que hoy se fortalece con la promesa de caminar juntos para siempre."
    }
  ],
  "faq": [
    {
      "id": "vestimenta",
      "question": "¿Cuál es el código de vestimenta?",
      "answer": "Formal elegante. Sugerimos tonos neutros, pasteles o colores sobrios para acompañar la estética de la celebración."
    },
    {
      "id": "acompanante",
      "question": "¿Puedo llevar acompañante?",
      "answer": "Por favor revisa tu invitación. Si incluye acompañante, podrás indicarlo al confirmar asistencia."
    },
    {
      "id": "ceremonia",
      "question": "¿Dónde será la boda?",
      "answer": "La celebración tendrá lugar en el Renaissance Santo Domingo Jaragua Hotel & Casino (av. George Washington núm. 367). La ceremonia será en el Palm Court y la recepción en el Salón Anacaona III. Puedes consultar la ubicación en Google Maps."
    },
    {
      "id": "parqueo",
      "question": "¿Hay parqueo?",
      "answer": "Sí, el hotel cuenta con parqueo privado y personal de apoyo para orientar a los invitados."
    },
    {
      "id": "fecha-limite",
      "question": "¿Hasta cuándo puedo confirmar?",
      "answer": "Agradecemos confirmar asistencia antes del 30 de julio de 2026."
    }
  ],
  "rsvp": {
    "title": "Confirma tu asistencia",
    "description": "Tu respuesta nos ayuda a preparar cada detalle con cariño.",
    "successMessage": "Gracias por confirmar. Hemos recibido tu respuesta.",
    "submitLabel": "Confirmar asistencia",
    "attendanceOptions": [
      {
        "label": "Sí asistiré",
        "value": "si"
      },
      {
        "label": "No podré asistir",
        "value": "no"
      }
    ]
  },
  "contact": {
    "email": "mbenzanroa@gmail.com",
    "phone": "+18294318077",
    "location": "Santo Domingo, República Dominicana"
  },
  "socialLinks": [
    {
      "id": "instagram",
      "label": "Instagram",
      "url": "https://instagram.com/"
    },
    {
      "id": "facebook",
      "label": "Facebook",
      "url": "https://facebook.com/"
    },
    {
      "id": "pinterest",
      "label": "Pinterest",
      "url": "https://pinterest.com/"
    }
  ],
  "theme": {
    "ivory": "#fbf6ec",
    "paper": "#fffdf8",
    "linen": "#f3eadb",
    "gold": "#b88a43",
    "ink": "#211b17",
    "muted": "#6f655d"
  },
  "liveMessagesEnabled": false,
  "gifts": {
    "message": "Estamos muy agradecidos de compartir nuestra historia contigo y de recibir con amor la bendición que Dios ponga en tu corazón.",
    "bank": "Banco BHD",
    "accountType": "Cuenta de ahorros en pesos",
    "accountNumber": "32860570010",
    "accountHolders": "Marcos Michelén / Ana Lapaix",
    "accountHolderId": "001-1943043-7",
    "registryUrl": "https://www.amazon.com/wedding/guest-view/2AH1AGVHIGAUN"
  }
}

export const defaultWeddingContent = weddingData

export default weddingData
